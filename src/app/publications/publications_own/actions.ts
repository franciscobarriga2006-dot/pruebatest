...
function convertDecimalFields(obj: any): any {
  ...
  if (typeof obj === "object") {
    ...
    for (const k in obj) {
      const v = obj[k];
      if (v && typeof v === "object" && typeof (v as any).toNumber === "function") out[k] = (v as any).toNumber();
...
export async function GetPublications(filtro: Filtro = "todo") {
  ...
  if (filtro === "buscar") data = await propias();
    ...
    data = Array.from(map.values()).sort((x, y) => (new Date(y.fechaPublicacion || 0).getTime() - new Date(x.fechaPublicacion || 0).getTime()));
  }
...
const updateSchema = z
  ...
  .refine((d) => Object.keys(d).some((k) => !["id"].includes(k)), { message: "Sin cambios" });

export type UpdateResult =
  | { ok: true; id: number }
  | { ok: false; error: string; fieldErrors?: Record<string, string[]> };

export async function UpdatePublication(input: FormData | Record<string, any>): Promise<UpdateResult> {
  const session = await getSession();
  ...
  if (parsed.data.descripcion !== undefined) d.descripcion = parsed.data.descripcion;
  if (parsed.data.remuneracion !== undefined) d.remuneracion = parsed.data.remuneracion;
  if (parsed.data.tipo !== undefined) d.tipo = parsed.data.tipo;
  if (parsed.data.estado !== undefined) d.estado = parsed.data.estado;
  if (parsed.data.categoriaId !== undefined) d.categoriaId = parsed.data.categoriaId;
  if (parsed.data.ubicacionId !== undefined) d.ubicacionId = parsed.data.ubicacionId;
  if (parsed.data.fechaCierre !== undefined) d.fechaCierre = parsed.data.fechaCierre;
...
export async function DeletePublication(input: FormData | { id: number } | number): Promise<DeleteResult> {
  const session = await getSession();
  if (!session?.sub) return { ok: false, error: "No autenticado" };
  const userId = Number(session.sub);
  if (!Number.isInteger(userId)) return { ok: false, error: "Sesión inválida" };
  ...
  try {
    ...
    if (err?.code === "P2003") {
      ...
      return { ok: false, error: "No se puede eliminar: existen registros relacionados." };
    }
